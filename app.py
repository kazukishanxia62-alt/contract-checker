...
st.caption("契約書の写真をAIが確認し、記入漏れ・条件違反・要注意項目をチェックします。")
...
CHECK_ITEMS = {
    ...
    "1枚目": [
        ...
        (
            "雇用形態",
            """
            雇用形態の選択肢のいずれかに○などの選択がされているか確認する。
            """
        ),
        ...
        (
            ...
            """
            「世帯主の確認」欄について、
            申込者本人かその他か、必要な選択・記入がされているか確認する。

            ただし、
            「世帯主の確認」の右側にある「連絡先」欄は
            空欄でも問題ない。

            この連絡先が空欄であることを理由にNGにしてはいけない。
            """
        ),
        ...
        (
            ...
            reason に「10万円を超えているため要注意」と記載する。

            金額欄自体が本来記入対象なのに空欄なら missing。
            """
        ),
    ...
    "2枚目": [
        ...
        (
            ...
            印刷上明らかに記入必須と分かる欄、
            「※」等で必須と示されている欄のうち、
            上記チェック項目に含まれていない重大な記入漏れがないか確認する。
...
st.info(
    "契約書全体が写るように撮影してください。"
    "文字が小さすぎる場合は、できるだけ真上から近づいて撮影してください。"
)
        ...
        try:
            ...
            if (
                ...
                if total_missing > 0:
                    ...
                    st.error(
                        f"❌ 記入漏れの可能性：{total_missing}件"
                    )

                if total_warning > 0:

                    st.warning(
                        f"⚠️ 要注意項目：{total_warning}件"
                    )

                if total_uncertain > 0:

                    st.warning(
                        f"🔍 AIでは判定できない項目：{total_uncertain}件"
                    )
